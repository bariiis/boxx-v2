...
export default function HookableAiCta3({
  ...
  const getVisibleItems = () => {
    const items = [];
    for (let i = -2; i <= 2; i++) {
      const index = (currentIndex + i + painPoints.length) % painPoints.length;
      items.push({ text: painPoints[index], position: i, index });
    }
    return items;
  };
  ...
  const getOpacity = (position: number) => {
    switch (position) {
      case 0: return 1;
      case -1: case 1: return 0.4;
      case -2: case 2: return 0.15;
      default: return 0;
    }
  };

  const getTranslateY = (position: number) => position * 70;
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8">
          ...
          <div className="relative h-[280px] md:h-[350px] w-full md:w-auto md:min-w-[400px] lg:min-w-[500px] overflow-hidden">
            <div
              ...
              style={{ background: `linear-gradient(to bottom, ${colors.background} 0%, transparent 100%)` }}
            />
              ...
              <AnimatePresence mode="popLayout">
                {getVisibleItems().map((item) => (
                  <motion.div
                    ...
                    initial={{ opacity: 0, y: getTranslateY(item.position + 1) }}
                    animate={{ opacity: getOpacity(item.position), y: getTranslateY(item.position) }}
                    exit={{ opacity: 0, y: getTranslateY(item.position - 1) }}
                  ...
                  >
                    <span
                      ...
                      style={item.position === 0 ? gradientStyle : { color: mode === "dark" ? "rgba(255,255,255,0.3)" : undefined }}
                    >
                      {item.text}
                    </span>
                  </motion.div>
            ...
            <div
              ...
              style={{ background: `linear-gradient(to top, ${colors.background} 0%, transparent 100%)` }}
            />
          </div>
        </div>
      </div>
      ...
      <motion.div
        ...
        style={{ background: `linear-gradient(90deg, transparent 0%, rgba(0,0,0,0.1) 20%, rgba(0,0,0,0.1) 80%, transparent 100%)` }}
      />
    </section>
  );
}
