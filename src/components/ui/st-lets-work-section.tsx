...
export function LetsWorkTogether({
  ...
  successTitle = "Let's talk",
  successSubtitle = "Perfect",
  bookCallText = "Book a call",
  bookCallUrl = "https://cal.com",
  bookCallNote = "15 min intro call",
  ...
  const handleBookCall = () => {
    if (bookCallUrl) window.open(bookCallUrl, "_blank")
  }
      ...
      <div className="relative flex flex-col items-center gap-12">
        ...
        >
          ...
          <button
            ...
            style={{
              transform: showSuccess
                ? isButtonHovered ? "translateY(0) scale(1.02)" : "translateY(0) scale(1)"
          ...
          >
            ...
            >
              <Calendar className="size-4 transition-all duration-500 sm:size-5" strokeWidth={1.5} style={{ color: isButtonHovered ? "var(--background)" : "var(--foreground)" }} />
              <span className="text-sm font-medium tracking-wide transition-all duration-500 sm:text-base" style={{ color: isButtonHovered ? "var(--background)" : "var(--foreground)" }}>
                {bookCallText}
              </span>
              <ArrowUpRight className="size-4 transition-all duration-500 sm:size-5" strokeWidth={1.5} style={{ color: isButtonHovered ? "var(--background)" : "var(--foreground)", transform: isButtonHovered ? "translate(3px, -3px) scale(1.1)" : "translate(0, 0) scale(1)" }} />
            </div>
            <div
              className="h-px w-8 bg-border transition-all duration-500 sm:w-12"
              style={{ transform: isButtonHovered ? "scaleX(0)" : "scaleX(1)", opacity: isButtonHovered ? 0 : 0.5 }}
          ...
          <span
            className="text-xs uppercase tracking-widest text-muted-foreground/50 transition-all duration-500"
        ...
        >
          ...
          <span className="text-sm font-medium uppercase tracking-widest text-muted-foreground">{statusText}</span>
        </div>
        ...
        >
          <div className="flex flex-col items-center gap-6">
            ...
            >
              <span className="block overflow-hidden">
                <span className="block transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]" style={{ transform: isHovered && !isClicked ? "translateY(-8%)" : "translateY(0)" }}>
                  {headlineLine1}
                </span>
              </span>
              <span className="block overflow-hidden">
                <span className="block transition-transform delay-75 duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]" style={{ transform: isHovered && !isClicked ? "translateY(-8%)" : "translateY(0)" }}>
                  <span className="text-muted-foreground/60">{headlineLine2}</span>
                </span>
              </span>
            </h2>
          ...
          <div className="absolute -left-8 top-1/2 -translate-y-1/2 sm:-left-16">
            <div className="h-px w-8 bg-border transition-all duration-500 sm:w-12" style={{ transform: isClicked ? "scaleX(0) translateX(-20px)" : isHovered ? "scaleX(1.5)" : "scaleX(1)", opacity: isClicked ? 0 : isHovered ? 1 : 0.5 }} />
          </div>
        ...
        >
          ...
          {email && <span className="text-xs uppercase tracking-widest text-muted-foreground/60">{email}</span>}
        </div>
      </div>
    </section>
  )
